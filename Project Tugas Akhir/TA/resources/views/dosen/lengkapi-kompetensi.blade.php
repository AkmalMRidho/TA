...
<div class="container mt-1">
    ...
    <form action="{{ route('lengkapi-kompetensi.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
	<div id="dynamic_form">
        ...
        <div class="form-group mb-4">
            <label for="mata_kuliah" class="form-label">Mata Kuliah yang diampuh:</label>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        function updateRemoveButtons() {
            document.querySelectorAll('#mata_kuliah_wrapper .remove-input').forEach(button => {
                button.style.display = (document.querySelectorAll('#mata_kuliah_wrapper .input-group').length > 1) ? 'inline' : 'none';
            });
