...
<div class="container mt-2">
    ...
    <form action="{{ route('lengkapi-kompetensi-kokape.update', ['id' => $tendik->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
    ...
    <div class="row">
        ...
        <!-- Lama Menjabat Section -->
        <div class="form-group mb-2">
            <label for="lama_jabatan" class="form-label">Lama Menjabat :</label>
            <select name="lama_jabatan" class="form-control" required>
                <option value="" disabled selected>Pilih lamanya menjabat</option>
                @for ($i = 1; $i <= 30; $i++)
                    <option value="Dosen selama {{ $i }} tahun" {{ $tendik->lama_jabatan == "Dosen selama $i tahun" ? 'selected' : '' }}>Dosen selama {{ $i }} tahun</option>
                @endfor
            </select>
        </div>
